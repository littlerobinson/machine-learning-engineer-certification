...
if __name__ == "__main__":
    ...
    try:
        ...
        # df_list_csv, df_list_json = dw_rds.get_dataframes_from_s3_dir(bucket_name=os.environ['AWS_BUCKET_NAME'], s3_dir=os.environ['AWS_PROJECT_PATH'])
        # print(df_list_csv)
        # print(df_list_json)

        city_geo_df = dw_rds.read_csv_from_s3(bucket_name=os.environ['AWS_BUCKET_NAME'], key=os.environ['AWS_PROJECT_PATH'] + "/city_geo_infos.csv")
        city_geo_df = dw_rds.clean_and_save_city_df(city_geo_df)
        print(city_geo_df)

        forecast_df = dw_rds.read_csv_from_s3(bucket_name=os.environ['AWS_BUCKET_NAME'], key=os.environ['AWS_PROJECT_PATH'] + "/weather_infos.csv")
        forecast_df = dw_rds.clean_and_save_forecast_df(forecast_df)
        print(forecast_df)

        accomodation_df = dw_rds.read_json_from_s3(bucket_name=os.environ['AWS_BUCKET_NAME'], key=os.environ['AWS_PROJECT_PATH'] + "/booking_results.json")
